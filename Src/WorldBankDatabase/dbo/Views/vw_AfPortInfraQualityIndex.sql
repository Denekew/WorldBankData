create view vw_AfPortInfraQualityIndex
AS
...
CTE_MaximumValuePercent(CountryName, CountryCode, PortInfraQualityIndex)
AS(
	select CountryName,
		CountryCode,
		CASE
		   ...
		   -- ELSE YR2014
		END AS PortInfraQualityIndex
	From CTE_AnnualValuePercent
)
SELECT CountryName, CountryCode, PortInfraQualityIndex
FROM CTE_MaximumValuePercent
WHERE PortInfraQualityIndex is not null and PortInfraQualityIndex > 0 
